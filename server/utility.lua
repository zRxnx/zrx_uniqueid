...
local resourceName = 'zrx_uniqueid'
...
local getRepoInformations = function()
    ...
    PerformHttpRequest('https://api.github.com/repos/zRxnx/zrx_uniqueid/releases/latest', function(err, response)
        if err == 200 then
            ...
            repoURL = 'https://github.com/zRxnx/zrx_uniqueid'
        end
    end, 'GET')
...
CreateThread(function()
    if curResName ~= 'zrx_uniqueid' then
        resourceName = ('zrx_uniqueid (%s)'):format(curResName)
    end

    while continueCheck do
        PerformHttpRequest('https://github.com/zRxnx/zrx_uniqueid/releases/latest', checkVersion, 'GET')
        Wait(3600000)
    end
end)